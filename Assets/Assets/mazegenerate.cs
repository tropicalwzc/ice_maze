...
public class mazegenerate
{
    ...
    void connect(char [,]maze,int startx,int starty,int endx,int endy,int height,int width,int tower=100)
    {
        tower--;
        if (tower < 0)
            return;
        ...
        int midx = (startx+endx) / 2+Random.Range(-1,2);
        if (midx < 1)
            midx = 1;
        if (midx > height - 2)
            midx = height - 2;
        int midy = (starty+endy) / 2+Random.Range(-1,2);
        if (midy > width - 2)
            midy = width - 2;
        if (midy < 1)
            midy = 1;
    ...
    void copymaze(char[,]maze,char [,]extmaze,int size = 500)
    {
    ...
    public void generatemaze(char[,] maze, char[,] answer, int mazeheight, int mazewidth)
    {
        ...
        int midingx = Random.Range(1, mazeheight-1);
        int midingy = Random.Range(1, mazewidth -1);
        ...
        {
            midingx = Random.Range(1, mazeheight - 1);
            midingy = Random.Range(1, mazewidth - 1);
        }
